...
module.exports = {
    ...
    create: context => {
        ...
        /**
         * Send report if needed after object and array walking
         */
        const sendObjectReport = (literals, nameUpper, node) => {
            if (lastValue === false && nameUpper) {
                context.report(node, LOWER);
            } else if (!literals && nameUpper) {
                context.report(node, LOWER);
            } else if (literals && !nameUpper && lastValue !== false) {
                ...
                if (node.kind === 'const') {
                    ...
                    node.declarations.forEach(variableDeclarator => {
                        ...
                        if (initType === 'ObjectExpression') {
                            ...
                            sendObjectReport(literalInObject, nameUpper, node);

                        } else if (initType === 'ArrayExpression') {

                            const literalInArray = getLiteralsInObject(init.elements);
                            sendObjectReport(literalInArray, nameUpper, node);
